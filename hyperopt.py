...
It is basically brute-force grid search.
"""
...
"""
In order to use docopt, activate the input
and replace the 'pass' statement with the code
...
if len(sys.argv) < 2:  # or sys.argv[1] not in ["bnc","wiki","rawiki","w2v","1k","5k","10k"]:
    print("Check your parameters! Parameter sequence: \n\
        ...
        -logto [directory]    one file in [directory] per run; default: log/hyperopt/default_log\n\
...
def get_text_resources_from_argv():
    if sys.argv[1] == "bnc":
        ...
        column_labels = "/home/simon.preissner/FFP/ukwac_100m/ukwac.w2v.400.vocab" #TODO change all of these paths!
    elif sys.argv[1] == "1k":
    ...
    for kc_factor in range(kc_factor_min, kc_factor_max + 1, kc_steps):
        for projections in range(projections_min, projections_max + 1, proj_steps):
            for hash_size in range(hash_perc_min, hash_perc_max + 1, hash_steps):
                # instantiate and apply a fruitfly
                # for working with the same fruitfly every time, use fruitfly = Fruitfly.from_config(...)
                run += 1
